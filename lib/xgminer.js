...
xgminer.prototype.enableGpu = function (deviceId) {
  var deferred = Q.defer();
  var self = this;

  if (!deviceId)
    deferred.reject(util.createError('Invalid Device ID', 'You need to specify a device ID to enable.'));

  self.send('gpuenable', deviceId).then(function (res) {
...
xgminer.prototype.disableGpu = function (deviceId) {
  var deferred = Q.defer();
  var self = this;

  if (!deviceId)
    deferred.reject(util.createError('Invalid Device ID', 'You need to specify a device ID to disable.'));

  self.send('gpudisable', deviceId).then(function (res) {
    deferred.resolve(res);
  }, function (err) {
    deferred.reject(err);
  });
